import os
# LLM（OpenAI）のライブラリが app.models.__init__.py 経由で初期化される際のエラーを回避するため
# ダミーのAPIキーを環境変数にセットしておく
...
from app.models.tts.style_bert_vits2_tts import StyleBertVITS2_TTS
...
def main():
    ...
    text = (
        ...
        "本学は創立以来、自由の学風を重んじ、多様な価値観を尊重してまいりました。"
        "これからの変化の激しい時代において、自ら考え、行動する力がより一層求められます。"
        "皆さんがここで培った知性と豊かな人間性を活かし、広く社会に貢献されることを期待しています。"
    )
    # =====================================================================
    ...
    # 音声合成モデルの初期化
    try:
        tts = StyleBertVITS2_TTS(str(tts_model_dir))
